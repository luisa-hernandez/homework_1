	...
	public static void main (String[] args) {
			...
			try {
				...
				while(input.hasNextLine()){
					...
					Course myCourse = new Course(data[0], data[1],Integer.parseInt(data[2]),Integer.parseInt(data[3]), data[5],Integer.parseInt(data[6]),data[7]);
					//adds data to courseList
					courseList.add(myCourse);	
					
					//this is for testing and printing out what is contained in myCourse
					//myCourse.print();
				}
				...
				if (answer == "admin"){
					...
					if (username == "Admin" && password == "Admin001"){
						...
						while(choice != 'q'){
							...
							switch (choice){
								case 'a': admin.addCourse(courseList); 
									break;
								case 'c': admin.displayCourse(courseList);
									break;
								case 'd': admin.deleteCourse(courseList);
									break;
								case 'e': admin.editCourse(courseList);
									break;
								case 'q': 
									break;
								case 'r': admin.registerStudent(courseList);
									break;
								case 't': admin.reports(courseList);
									break;
								}
							}
					}	
				...
				if (answer == "student"){
					...
					System.out.println("Password: ");
					String password = type.nextLine();
					//if the username and password are correct, follow through with these options
					if (username == "Admin" && password == "Admin001"){
						char choice = 'a';
						...
						while(choice != 'q'){
							...
							choice = sc.next().charAt(0);
							switch (choice){
								case 'e': student.enrolledCourse(courseList); 
									break;
								case 'f': student.nonfullCourse(courseList);
									break;
								case 'r': student.registerCourse(courseList);
									break;
								case 'v': student.viewCourse(courseList);
									break;
								case 'w': student.withdrawCourse(courseList);
									break;
								}
							}
					}
